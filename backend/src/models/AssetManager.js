...
class AssetManager extends AbstractManager {
  ...
  setIdIdea(assetsIds, idIdea) {
    return this.database.query(
      `UPDATE ${this.table} SET id_idea = ? WHERE ${
        this.id
      } IN (${assetsIds.join(",")}) AND id_idea IS NULL`,
      [idIdea]
    );
  }
  ...
  setIdComment(assetsIds, idComment) {
    return this.database.query(
      `UPDATE ${this.table} SET id_comment = ? WHERE ${
        this.id
      } IN (${assetsIds.join(",")}) AND id_comment IS NULL`,
      [idComment]
    );
  }
  ...
  setIdChallenge(assetsIds, idChallenge) {
    return this.database.query(
      `UPDATE ${this.table} SET id_challenge = ? WHERE ${
        this.id
      } IN (${assetsIds.join(",")}) AND id_challenge IS NULL`,
      [idChallenge]
    );
  }
}
